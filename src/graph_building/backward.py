...
def get_bck_probs_action_mult(trajs, _traj_lens, _actions, _raw_embeddings, _embedding_structure, _bck_models, action_type=0, n=1, **_kwargs):
    ...
    for i, ((__, a), (s, __)) in enumerate(zip(trajs[:-1], trajs[1:])):
        ...
        if action_type == 0:  # edge
            num_edges = int(torch.sum(s[1][:, :, 1] == 1, dim=(0, 1)).item())
            ...
            single_state_p_b_std.append(0 if num <= 1 else torch.tensor([n]*num_edges + [1]*(num-num_edges), dtype=float).std())  # slow but oh well

        elif action_type == 1:  # node
            if a == len(s[2])**2:  # if action is add node, assign more weight
                log_p_b[i+1] = math.log(n/(num+n-1))
            else:  # otherwise, assign normal weight
                num_nodes = torch.sum(torch.sum(s[0], dim=1) > 0, dim=0)
            ...
            single_state_p_b_std.append(0 if num <= 1 else torch.tensor([n] + [1]*(num-1), dtype=float).std())
        ...
        single_state_p_b_std = torch.tensor(single_state_p_b_std, dtype=float)

        return log_p_b, None, {"mean_single_state_p_b_std": single_state_p_b_std.mean().item(),
                               "std_single_state_p_b_std": 0 if len(single_state_p_b_std) == 1 else single_state_p_b_std.std().item()}
...
def get_bck_probs_aligned(trajs, _traj_lens, _actions, _raw_embeddings, _embedding_structure, _bck_models,
                          cor_val=0.8, inc_val=0.05, reward_arg=0.8, reward_idx=1, **_kwargs):
    ...
    log_p_b = torch.tensor([
        get_aligned_action_log_prob(*s, a, reward_idx=reward_idx, reward_arg=reward_arg,
                                    correct_prob=cor_val, incorrect_prob=inc_val) for s, a in trajs
    ])
...
def get_bck_loss_free(log_z, traj_log_p_f, _log_rewards, traj_log_p_b, _info, **_kwargs):
    return torch.tensor([0.]*len(traj_log_p_f), device=log_z.device), traj_log_p_b
...
def get_bck_probs_soft_tlm(trajs, _traj_lens, actions, raw_embeddings, embedding_structure, bck_models, a=0.5, **_kwargs):
    ...
    return (
        (1-a) * tlm_log_p_b + (a) * uniform_log_p_b,
        None,
        {"mean_single_state_p_b_std": single_state_p_b_std.mean().item() * (1-a), "std_single_state_p_b_std": single_state_p_b_std.std().item() * (1-a)}
    )

def get_bck_probs_smooth_tlm(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, a=0.5, device="cuda", **_kwargs):
    bck_action_probs = get_action_probs(*raw_embeddings[0], *embedding_structure[0], *bck_models, random_action_prob=0, apply_masks=False)
    single_state_p_b_std = torch.std(bck_action_probs, dim=1)
    tlm_log_p_b = bck_action_probs[list(range(len(bck_action_probs))), torch.roll(actions, 1, 0)].to(device)
    with torch.no_grad():  # necessary?
        uniform_log_p_b = torch.tensor([-math.log(get_num_previous_acts(s)) for s, _a in trajs], device=device)
...
def get_bck_probs_biased_tlm(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, multiplier=5, ns=[3, 4, 5], device="cuda", **_kwargs):
    ...
    biased_tlm_log_p_b = tlm_log_p_b * (1 + (multiplier - 1) * torch.tensor([int(is_n_connected(*g, ns=ns, connected=False)) for g in final_graphs], device=device)).repeat_interleave(traj_lens)
...
def get_bck_probs_meta(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, weights=None, reward_arg=0.8, device="cuda", **_kwargs):
    ...
    c_0_v, __, c_0_m = get_bck_probs_const(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, value=0.15, device=device)
    u_0_v, __, u_0_m = get_bck_probs_uniform(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, device=device)
    a_0_v, __, a_0_m = get_bck_probs_action_mult(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, action_type=0, n=2, device=device)
    a_1_v, __, a_1_m = get_bck_probs_action_mult(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, action_type=0, n=0.5, device=device)
    a_2_v, __, a_2_m = get_bck_probs_action_mult(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, action_type=1, n=2, device=device)
    a_3_v, __, a_3_m = get_bck_probs_action_mult(trajs, traj_lens, actions, raw_embeddings, embedding_structure, bck_models, action_type=1, n=0.5, device=device)
    ...
    back_probs = torch.transpose(torch.stack([c_0_v, u_0_v, a_0_v, a_1_v, a_2_v, a_3_v, a_4_v, a_5_v, r_0_v, l_0_v, t_0_v]).to(device), 0, 1)
    return torch.matmul(back_probs, weights.to(device)), traj_log_p_b, joint_metrics
...
const = (get_bck_probs_const, lambda *pargs, **_kwargs: (torch.tensor([0.]*len(pargs[1]), device=pargs[0].device), None))
uniform = (get_bck_probs_uniform, lambda *pargs, **_kwargs: (torch.tensor([0.]*len(pargs[1]), device=pargs[0].device), None))
action_mult = (get_bck_probs_action_mult, lambda *pargs, **_kwargs: (torch.tensor([0.]*len(pargs[1]), device=pargs[0].device), None))
rand = (get_bck_probs_rand, lambda *pargs, **_kwargs: (torch.tensor([0.]*len(pargs[1]), device=pargs[0].device), None))
aligned = (get_bck_probs_aligned, lambda *pargs, **_kwargs: (torch.tensor([0.]*len(pargs[1]), device=pargs[0].device), None))
