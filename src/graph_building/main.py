...
from gfn import (
    ...
    get_tb_loss_action_mult,
    get_tb_loss_soft_tlm,
    get_tb_loss_smooth_tlm,
    get_tb_loss_loss_aligned,
...
parser.add_argument("-b", "--reward-arg", type=float, default=0.8, help="(base for exponent used in reward calculation)")
...
parser.add_argument("-x", "--edges-first", action="store_true", default=False, help="whether to add edges before nodes in precomputed trajectories")
...
parser.add_argument("-o", "--backward-reset-period", type=int, default=-1, help="how often to reset the backward policy (-1 for no resets)")
...
configs = {
    "tb-const": (  # uniform backward policy with a constant (unnormalised) backward probability
    ...
    "tb-uniform-inc-action": (  # uniform backward policy with an added bias towards a specific action
        lambda *pargs, **kwargs: get_tb_loss_action_mult(*pargs, action_type=args.loss_arg_a, n=args.loss_arg_b, **kwargs),
        {"parameterise_backward": False}
    ),
    "tb-uniform-rand": (  # backward probabilities resampled from a normal distribution on each application
        lambda *pargs, **kwargs: get_tb_loss_rand(*pargs, std=args.loss_arg_a, **kwargs),
        {"parameterise_backward": False}
    ),
    "tb-aligned": (  # aligned to handmade backward policy
        lambda *pargs, **kwargs: get_tb_loss_aligned(*pargs, reward_arg=args.reward_arg, correct_val=args.loss_arg_a, incorrect_val=args.loss_arg_b, **kwargs),
        {"parameterise_backward": False}
    ),
    "tb-frozen": (  # frozen parameterised backward policy
        get_tb_loss_frozen,
    ...
    "tb-soft-tlm": (  # TLM / pessimistic mixed with a uniform distribution
        lambda *pargs, **kwargs: get_tb_loss_soft_tlm(*pargs, a=args.loss_arg_a, **kwargs),
        {"parameterise_backward": True}
    ),
    "tb-smooth-tlm": (  # TLM / pessimistic mixed with a uniform distribution (pre-backprop)
        lambda *pargs, **kwargs: get_tb_loss_smooth_tlm(*pargs, a=args.loss_arg_a, **kwargs),
        {"parameterise_backward": True}
    ),
    ...
    "tb-loss-aligned": (  # aligned to handmade backward policy
        lambda *pargs, **kwargs: get_tb_loss_loss_aligned(*pargs, iters=args.loss_arg_a, std_mult=args.loss_arg_b, **kwargs),
        {"parameterise_backward": False}
    )
}
...
# TODO: different node history bounds
...
if __name__ == "__main__":
    ...
    for it, (jagged_trajs, log_rewards) in zip(range(args.num_batches), data_loader):
        ...
        loss, metrics = get_loss(jagged_trajs, log_rewards, base_models, log_z_model, *fwd_models, *bck_models, constant_log_z=args.log_z, device=args.device)
        loss.backward()
        ...
        if parameterise_backward:
            ...
            if args.loss_fn not in ["tb-free", "tb-max-ent"]:
        ...
        with torch.no_grad():
            ...
            if args.save and (it+1)%args.cycle_len == 0:  # TODO: THIS!!!
                ...
                #    fwd_action_probs, bck_action_probs, metrics = get_action_log_probs_test_helper(nodes, edges, masks, args.loss_fn,
                #                                                                          base_models, fwd_models, bck_models, log_z_model,
                ...
                #    - embedding vector of forward and backward policies of full 1024 full trajectories
                #    - metrics (particularly fwd and bck loss; divergence) on the 1024 trajectories
                #    - sample of 1024 trajectories generated
                ...
                # record tanimoto similarity and mean reward for top k dissimilar graphs
                # record distance from uniform in backward probabilities
                # also save results w linear perturbations as a function of action type, num nodes, connectivity, ...
                ...
                if False:
                    ...
                    for i in range(8):
                        ...
                        loss, metrics = get_loss(
                            jagged_trajs, log_rewards, base_models, log_z_model, *fwd_models, *bck_models, constant_log_z=args.log_z, device=args.device
                        )
